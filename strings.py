...
def help_text() -> str:
    return (
        ...
        f"  /newhard — Start hard round (random theme)\n"
...
def global_leaderboard_text(rows, title) -> str:
    """Global leaderboard — names are tg:// links so tapping opens the user's Telegram profile."""
    if not rows:
        return f"{ICO_TROPHY()} <b>{title}</b>\n\nNo scores yet — play /newgame to get started!"
    lines = [f"{ICO_TROPHY()} <b>{title}</b>", "━" * 26]
    for i, row in enumerate(rows):
        ...
        uid  = row.get("user_id")
        name = row["name"]
        name_tag = f'<a href="tg://user?id={uid}">{name}</a>' if uid else f"<b>{name}</b>"
