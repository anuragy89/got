...
async def _launch_hard(chat_id: int, ctx, theme_key: str = None) -> None:
    """Start a single /newhard session — 11x11 grid, 10-15 words, high pts.

    Pass theme_key to force a specific theme (e.g. from /newhard space).
    Omit (or pass None) for a random non-repeating theme.
    """
...
async def cmd_newhard(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Start a Hard Mode game — /newhard or /newhard [theme]"""
    chat = update.effective_chat
    user = update.effective_user
    if chat.type == ChatType.PRIVATE:
        await update.message.reply_text(
    ...
    if sessions.cooldown(chat.id):
        ...
        return
    # Parse optional theme argument — e.g. /newhard space or /newhard bollywood
    args      = ctx.args or []
    arg       = args[0].lower() if args else None
    theme_key = arg if arg and arg in THEMES else None
    if arg and not theme_key:
        # User typed a theme name that doesn't match — let them know
        theme_names = ", ".join(THEME_LIST)
        await update.message.reply_text(
            f"❓ Unknown theme <code>{arg}</code>.\n"
            f"Available themes: <code>{theme_names}</code>",
            parse_mode=ParseMode.HTML,
        )
        return
    await _launch_hard(chat.id, ctx=ctx, theme_key=theme_key)
