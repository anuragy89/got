...
from game import sessions, GameSession, get_level, MAX_ROUNDS, pick_random_theme, pick_next_round_theme, touch_activity, idle_seconds
...
async def _end_round(chat_id, session, ctx, from_timer=False):
    ...
    # Pick a fresh theme for next round (different from current)
    next_theme = pick_next_round_theme(chat_id, session.theme, THEME_LIST) if not is_final else session.theme

    text = round_end(summary, missed, THEMES[session.theme]["name"],
                     session.round_num, MAX_ROUNDS, round_complete=round_complete)
    ...
    # Collect all message IDs to delete (grid + all in-round bot messages)
    all_msg_ids = []
    ...
    # Delete all old round messages after 2 minutes
    if all_msg_ids:
        async def _delete_msgs_later(msg_ids):
        ...
        asyncio.create_task(_delete_msgs_later(all_msg_ids))
...
_NUDGE_CHECK_INTERVAL = 30 * 60  # check every 30 minutes

async def idle_nudge_job(ctx: ContextTypes.DEFAULT_TYPE):
    """Scheduled job — sends a random nudge to groups idle for 3+ hours."""
    group_ids = await db.all_group_ids()
    for chat_id in group_ids:
        if sessions.active(chat_id):
            continue   # game running — skip
        idle = idle_seconds(chat_id)
        if idle < _IDLE_THRESHOLD:
            continue
        msg_text = random.choice(IDLE_NUDGES)
        try:
            await ctx.bot.send_message(chat_id, msg_text, parse_mode=ParseMode.HTML)
            touch_activity(chat_id)   # reset so we don't spam every check
        except Forbidden:
